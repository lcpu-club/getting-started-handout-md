---
icon: material/flash
---
# 知识速查

虽然笔者本人非常建议同学们通读本手册，但也理解有些同学可能只是想快速查阅知识点或获取某些技能。因此，笔者在此提供一个知识速查表，方便同学们快速定位到相关章。

## 配置相关环境

- 在纯 Windows 上搭建 C/C++ 编程环境：见《在纯Windows上搭建C/C++编程环境》
- 在纯 Windows 上搭建 Python 编程环境：见《Python、虚拟环境及其配置》
- 在 WSL 上搭建 C/C++ 编程环境：见《用WSL配置C/C++环境》

## 相关工具使用

- 正则表达式：见《正则表达式》
- VS Code：见《IDE及其选择》
- Conda：见《Python、虚拟环境及其配置》
- Git：见《Git与版本控制》
- GitHub：见《GitHub》
- SSH 密钥：见《SSH密钥》
- GPG 密钥：见《进阶：GPG密钥和信任网络》
- Markdown：见《Markdown》
- LaTeX：见《LaTeX》
- Typst：见《Typst》
- vim：见《VIM》
- Shell 命令行：见《常用终端命令行辞典》
- CMake：见《CMake》
- XMake：见《XMake》
- Meson：见《Meson》
- GDB 调试器：见《使用调试器》
- LLM、Vibe：见《LLM、VLM和提示词工程》和《Vibe Coding》
- tmux 和 screen：见《轻量级隔离：tmux和screen》
- Docker：见《重量级隔离：Docker》，以及 [官方文档](https://docs.docker.com/get-started/)
- uv：见 [uv 中文文档](https://uv.doczh.com/)
- pixi：见 [pixi 最新文档](https://pixi.sh/latest/)

## 其他优质教程汇总

- [PKU Getting Started](https://missing.lcpu.dev)
- [CS 自学指南](https://csdiy.wiki/)
- [中国科学技术大学 Linux 101](https://101.lug.ustc.edu.cn/)
- [清华大学 计算机系学生科协技能引导文档](https://docs.net9.org/)
- [星外之神的博客](https://wszqkzqk.github.io/tags)，内容庞杂而丰富
- [本书GitBook版本](https://lcpu-club.github.io/getting-started-handout-md/)，仍在施工中
- [Arthals的博客](https://arthals.ink/)，ICS学生的救星

## PKU 校内常用网址集合

- [网络服务](https://its.pku.edu.cn)
- [信息门户](https://portal.pku.edu.cn)
- [图书馆](https://lib.pku.edu.cn)
- [学生邮箱](https://mail.stu.pku.edu.cn)
- [选课网](https://elective.pku.edu.cn)
- [教学网](https://course.pku.edu.cn)
- [缴费系统](https://cwsf.pku.edu.cn)
- [教务网](https://www.dean.pku.edu.cn)
- [树洞](https://treehole.pku.edu.cn)
- [课程测评](https://courses.pinzhixiaoyuan.com)
- [正版软件](https://software.pku.edu.cn)
- [课程查询](https://class.wjsphy.top/)
- [笔记共享平台](https://pkuhub.cn/)，建议注册使用

<!-- handout-sync-source
source_type: latex
source_url: https://github.com/ZangXuanyi/getting-started-handout
source_branch: main
source_commit: e1f6c5d23fb730d144f0261027c515ca0971ffc6
source_commit_short: e1f6c5d
-->
