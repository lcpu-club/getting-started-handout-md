...
1. 将U盘插入上述"另一台计算机"。
2. 打开Rufus软件，选择你的U盘作为"设备"。
3. 点击"选择"按钮，选择下载好的操作系统镜像文件（ISO格式）。
...
打开你的目标计算机，并在启动时按下特定的按键，这个按键随着不同的主板而不同，常见的有F2、F10、Delete等。具体按键可以参考主板说明书或网上搜索你的主板型号加上"进入BIOS按键"。通常来说，不停地反复按上述按键总能蒙对。
...
1. 打开浏览器，访问Microsoft Office官网，下载Office安装程序。PKU提供了正版Office的下载和激活方式，这里可以前往[北京大学正版软件](software.pku.edu.cn)网站获取（需要北京大学的统一身份认证账号）。
2. 运行下载的安装程序，按照提示完成安装。PKU提供的是一个ISO镜像，我们需要右键该文件，选择"装载"，然后运行其中的安装程序，完成安装。
3. 安装完成后，打开任意Office组件（如Word），根据提示输入激活密钥进行激活。如果是PKU提供的Office，则可以按照网站上的说明进行激活，在笔者的印象中是下载一个小工具，在校园网环境下运行该工具即可完成激活。
4. 现在你可以开始使用Office了。
...
但也有例外，例如miniconda等软件，官方推荐的安装方式就是下载安装包[^1]，然后运行安装脚本进行安装。
...
- **下载器类**
  ...
  - wget：一个老牌、免费的命令行下载工具，支持HTTP、HTTPS和FTP下载。它可以通过命令行参数来控制下载行为，适合有一定技术基础的用户使用。具体使用见相关章节。
...
- **浏览器类**
  ...
  - Zen Browser：基于 Firefox 的轻量浏览器，界面更加简洁美观，同时支持自定义主题和插件。
- **压缩与解压缩类**
...
- **输入类**
  - Rime：一个高度可定制的输入法框架，可以自定义输入方式与习惯。
  - InputTip/ImTip：输入状态提示，可以显示当前处于中、英文状态等，前者也可以根据不同输入状态更换鼠标样式。
  - Keyviz/KeyCastOW：键鼠操作可视化，会在屏幕中显示按键操作，方便用户查看和学习。
  - ContextMenuManager：可视化的右键菜单管理器，通过图形界面操作注册表来编辑和管理右键菜单，避免菜单栏过长。
- **文件搜索与窗口管理**
  - Everything/Everything Toolbar：一个免费的文件搜索工具，能够快速地搜索计算机上的文件。它的搜索速度极快，支持多种搜索方式，包括模糊搜索、正则表达式搜索等，后者是任务栏集成的版本，操作更加方便。
  - Fences/Coodesker：桌面整理工具，可以按照文件类型进行整理。
  - Deskpins：任意窗口置顶，操作十分简单。
  - AltSnap：按住Alt可在任意位置拖动窗口。
  - Groupy：任意窗口的分组管理，可以将不同的窗口在上方显示与浏览器类似的标签条，方便窗口管理。
  - Office Tab：Office三件套的窗口管理，在工具栏下方显示所有打开的文档，方便切换与关闭。
  - OneCommander/Directory Opus：更高级的文件管理，具有批量命名、批量复制、批量移动等功能，操作更加方便。
- **效率与自动化**
  - uTools：极简的多功能应用，具有截图翻译、提取颜色、转换图片格式等插件，也有插件市场可以选择。
  - AutoHotkey：一个强大的自动化脚本工具，可以很方便地完成热键修改、文本替换、自定义宏等操作。
  - PowerToys：Windows 官方效率工具合集，包括保持唤醒、颜色选择器、键盘管理器以及鼠标工具等工具。
- **其他**
  ...
  - Ventoy：一个开源的u盘启动工具，能够使多个ISO镜像共存于U盘，而不必将单一镜像烧录于U盘。它能使多个镜像文件和U盘其他文件共存，是装机盘和资料盘合一的好工具。
  ...
  - Okular：一个免费的开源PDF阅读器，支持多种文档格式，包括PDF、EPUB等。较Foxit更为轻量。
  - TrafficMonitor：网速/CPU/内存悬浮窗，也可以嵌入任务栏中实时显示内存与CPU运行情况。
...
Server = https://mirrors.huaweicloud.com/archlinuxcn/$arch # 华为开源镜像站
```
...
2. Linux内核的基本结构和工作原理，例如进程管理、内存管理、文件系统等。
3. Linux的部署，例如用ansible等工具实现自动化安装和配置，理解声明式系统（如NixOS、Guix等）的原理和优势，并尝试使用它们。
4. Linux的安全机制，例如SELinux、AppArmor等。
5. 试着定制你的系统，创作出好玩的工具，并写PKGBUILD文件打包成AUR包，发布到AUR上。
