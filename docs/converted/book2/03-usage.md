...
当你看到这个标题时，一定会冒出上面的念头。打字不就是输入文字吗？要打"双"我们就按"shuang"，要打"你"我们就按"ni"。那么恭喜你已经解锁了最常用也是最符合大部分人使用习惯的输入方式（全拼）。所谓全拼就是需要打全音节的拼写。

很好，现在我们已经了解了全拼打字。但是经过仔细思考，我们可以发现一个重要的问题————和英文不同，汉字的拼音并不是由字母组成的啊！一个汉字的拼音是由一个声母以及一个韵母组成的，如果我们能直接输入声母和韵母，那么打字不就更轻松了吗。于是双拼也应运而生，这种打字方式是将键盘上的字母区映射到声母与韵母上，这样一来我们击键两次就可以打出一个汉字了！根据映射方案的不同，双拼也有不同的方案（例如自然码、小鹤双拼、微软双拼等）。以小鹤双拼为例，小鹤双拼的映射方案如下图。

![小鹤双拼映射方案](images/100/flypy.png)

我们来简单见识一下双拼的威力：请在任意文本编辑器中输入"镶嵌在窗框上的霜花正在悄悄融化"这句话。

全拼的键入是这样的：

```text
xiang/qian/zai/chuang/kuang/shang/de/shuang/hua/zheng/zai/qiao/qiao/rong/hua
```

双拼的键入是这样的：

```text
xl/qm/zd/il/kl/uh/de/ul/hx/vg/zd/qn/qn/rs/hx
```

我们可以明显地看出双拼打字所需的键入次数（我们称为"码长"）更少。包括这个优点，双拼方案与全拼方案的对比优势如下：

- 双拼方案的码长明显减少，打字速度也更快。
- 双拼能够减少一定的重码情况，例如"西安"与"先"、"余额"与"月"等。
- 双拼的码长基本固定在2，而全拼的码长则根据音节数而定，双拼打字更有节奏感。

**音码的不足在于同音字的处理**，一个拼音往往对应十几甚至几十个汉字（我们称为"重码"）。对于常见的字我们可能需要翻页才能找到对应的汉字，十分影响打字节奏。有没有办法避免打出同音字呢？有的，四大发明的活字印刷是将字直接印下来，那么我们也可以将字的偏旁输入然后选择对应的字啊！于是五笔输入法也应运而生，例如王码86五笔、郑码、虎码等方案。这里以王码86五笔为例，其映射方案如下图。

![王码86五笔映射方案](images/100/wang86.png)

这种根据字根字形进行输入的方法我们称为"形码"，可以有效减少同音字的影响而增加输入效率。不过随着拼音的普及，这种难度较高的输入形式使用人数也在减少（例如上世纪的打字员一职）。

而现在，为了再次降低音码的重码率，我们在双拼的基础上也可以引入辅助码，将简单的字形映射到键盘上（辅助码以音托为主，因此学习成本要小于五笔），再结合双拼就基本可以做到和五笔输入相似的效果了。常见的辅助码有自然码辅码、鹤形辅助码等。由于大部分输入法厂商没有配备辅助码，因此有兴趣的同学可以自行学习提升，这里不做展开。

以上是键盘对于中文的输入，这里笔者需要介绍一种特殊的英文输入形式————滑键输入。这种输入形式不需要一个一个键入单词的字母，而是可以连续划过字母直至你想要的单词上屏。这种输入形式可以提高输入英文的效率，在iOS输入法或Gboard中有相应的支持。
...
![标准指法打字示例](images/100/type.png)

有些人会问五笔和拼音哪个打字更快。实际上，这取决于你所使用的打字方案，对于打字员等专业人士而言，他们打字的时候眼睛只看着手写或打印的纸张，屏幕和键盘都不看；他们和抄写一样，直接复制字形并打出编码，因此五笔的速度会更快一些。但是，五笔是违反人语言习惯的输入法，需要对大量字根字形非常熟练，而我们在写东西的时候，往往是先想到词语的发音，然后再想到字形，而五笔则是直接根据字形来输入，这对于大多数人而言并不自然。

因此，对于大多数人而言，拼音输入法（这里指全拼与双拼）更为合适。另外，随着技术的不断进步，现代的大多数输入法连接了网络，能够通过用户的上文推测下文大概是什么词语，从而提高了输入效率，双拼+辅助码的组合也可以媲美五笔的极限速度。

目前大多数输入法都有特殊的输入功能，例如按下 `u` 即可进入生僻字输入模式，可以用于输入一些不常用的汉字；按下 `v` 可以输入英文符号等。我们建议同学们阅读自己使用的输入法的帮助文档，了解这些特殊功能，从而提高边界条件下的输入效率。

最后，无论你使用全拼、双拼或者五笔输入，你都需要明白打字是一个熟练工作，只有多练习才能提高打字速度和准确率。对于双拼以及五笔，我并不建议去背诵对应的映射方案，你只需要将你所有的输入形式改为对应输入方案并将手机屏保换为对应映射表，不出2个月，你就基本能拥有和全拼相近的输入效率了。在熟练之后，你可以练习盲打。打字较快的人大多数都会盲打，也就是在打字的时候眼睛仅盯着输入法选字窗口，而不看键盘。这样可以减少眼睛在键盘和屏幕之间的移动，提高打字效率。我们推荐使用一些打字练习软件，例如TypingClub、Keybr、金山打字通等，来提高打字速度和准确率。
...
在计算机上为了方便读取，之后的数据大小单位以 $2^{10}=1024$ 为倍数递增，即1KiB=1024B，1MiB=1024KiB，1GiB=1024MiB，1TiB=1024GiB。[^1]另外一套单位系统以1000为倍数递增，也就是1KB=1000B，1MB=1000KB，1GB=1000MB，1TB=1000GB。这种标定方式便于硬件的生产，因此通常由硬件厂商使用，我们在日常生活中看到的硬盘、U盘等存储设备的容量使用的就是这种方式。

因此购买的硬盘，计算机认为的容量会比标定容量小一些，例如一个标定1TB的硬盘：

$$ 1TB = 1000^{4} \times 1000^{8} B = \frac{1000^{12}}{1024^{4}} TiB \approx 0.9095 TiB = 931.32 GiB $$
...
实际上这些单位非常大，我们拿钱来举例子：可支配财产1M人民币（下简称元）的人（$10^6$）可以在中国大多数中小城市过上一个不错的生活；可支配财产1G元的人（$10^9$）已是巨富，大概在富豪榜上靠后的位置榜上有名；1T元（$10^{12}$）已经是天文数字，大致相当于一个中小国家的全年GDP，这个量级的财富想要个人持有，估计只有开通货膨胀笑话的时候才会出现；但全人类一年的生产总值还不到1个P（$10^{15}$）元（估计约0.7P元）。

但放到数据上，这些单位反而显得非常渺小。1个PB的数据量则并不能说是特别大，一个大型互联网公司（字节、阿里、腾讯等）每天处理的数据也大概就这么多。更大的单位，一般情况下我们也很难遇见了。再往下走，一个TB大致是一个家庭的数据量总和；一个GB大概是一部高清电影的量级（一部高清电影的大小大约是几GB到一百GB不等）；一个MB大概是一本电子书（纯文本50万字，实际上很多网文已经超过了1MB了）的量级。
...
一个常用的手段是使用搜索功能。Windows文件资源管理器提供了搜索功能，可以帮助我们找到文件。我们可以在文件资源管理器的右上角输入关键词，文件资源管理器会自动搜索当前目录及其子目录中的文件，并显示匹配的结果。然而，文件管理器的搜索面对大量文件时效率很低。这时候，我们可以使用一些第三方的搜索工具，例如Everything、Listary等。这些工具可以快速索引计算机中的文件，并提供快速的搜索功能。它们通常比文件管理器的搜索功能更快、更强大。
...
![钓鱼攻击示例，右侧这个页面的域名显然不是qq的域名，其界面也被不合理地拉伸了，属于典型的钓鱼页面。该钓鱼页面会诱骗用户输入QQ账号和密码，从而窃取用户的账号信息。](images/100/fishing-1.jpg)
...
例如，某网页地址为http://example.com/files.html，该网页上有很多文件的链接，我们想要下载所有的PDF文件，可以使用以下命令：
