...
!!! info "阅读材料"
    ...
    现代计算机的理论灵魂来自阿兰·图灵，他在1936年提出"图灵机"模型，明确了"可计算问题"与"不可计算问题"的界限，为计算机科学奠基：一般认为，如果一个问题的解法可以被形式化为一个算法，那么这个问题就是可计算的；否则就是不可计算的。很多问题是可计算的，但也有一些问题（如停机问题：是否存在一个算法，能够判断任意程序在任意输入下是否会停止运行）是不可计算的。

    而冯·诺依曼则通过提出程序与数据共存的存储结构，将计算机带入现实，这一"冯·诺依曼架构"至今仍是主流（虽然为了性能起见，目前计算机的CPU已经不完全遵循冯诺依曼架构了，也混入了哈佛架构等的思想）。20世纪40年代，电子计算机的概念逐渐成型。

    第一台通用电子计算机ENIAC于1946年2月14日诞生，采用电子管实现高速计算，标志着电子计算机时代的开启（但它使用的是哈佛架构而不是冯·诺依曼架构）。此后，晶体管、集成电路和微处理器相继问世，计算机不断向小型化、高性能的方向演进，并借图形界面与互联网普及至千家万户。如今，计算机已融入生活各个角落，并在人工智能、云计算等新技术推动下持续发展。
...
一个计算机的主机设备如下图所示。下面将会逐个介绍这些设备。

![台式机的主机设备](images/hardwares/hardware.jpg)

![笔记本计算机的主机设备](images/hardwares/laptop.jpg)
...
现代计算机的主要外存设备是硬盘。硬盘可以分为机械硬盘（HDD）和固态硬盘（SSD）。机械硬盘使用磁头在旋转的磁盘上读取和写入数据，而固态硬盘使用闪存芯片来存储数据。固态硬盘的读写速度比机械硬盘快得多，现在价格也便宜得多，但是使用寿命较短，且因为电荷流失等问题无法接受长期不通电等情况，不适宜作为长期存档介质[^1]，除非花高价买高端的企业级SSD，但仍需定期通电。
...
- **软盘**：一种老古董，使用磁性材料存储数据，容量最大为1.44MB，连这本书都存不下。现在软盘因为存储容量小、速度慢、易损坏等缺点，已经被淘汰了。
...
!!! tip "提示"
    从上述例子中我们可以看到，CPU、GPU、TPU等的通用性是依次降低的，而在特定任务上的性能则是依次提升的。这种"专用性换取通用性能"的设计思路，是计算机体系结构中的一个重要原则，其一个重要体现就是软件硬件化，这也是近些年来兴起的"硬件加速器"设计思路的基础。
...
按性能，笔记本电脑可以简单分类为以下几类：

- **轻薄本**：轻薄本是指重量轻、厚度薄的笔记本电脑，通常用于日常办公、学习和娱乐。它们通常配备低功耗处理器，续航时间较长，但性能相对较弱。
- **游戏本**：游戏本通常配备了高性能处理器和独立显卡，能够运行大型游戏和图形密集型应用。它们通常较重，续航时间较短，但性能强大。很多人反映，背着这玩意去教室困难，请谨慎选择。另外，此类笔记本通常配备了花里胡哨的灯效，可能会引起不必要的误会。
- **全能本**：全能本是指兼顾轻薄和性能的笔记本电脑，配置不固定，总体性能介于轻薄本和游戏本之间，能够满足日常办公、学习和娱乐的需求，同时也具备一定的游戏性能，属于一种折中方案。
- **移动工作站**：移动工作站通常是指那些高性能且功能往往极为特化的笔记本电脑，主要面向专业用户，如设计师、工程师、科学家等。它们通常配备了高性能处理器、大容量内存和专业级显卡，且其上搭载的软件专门为这些硬件做出了优化，以满足专业用户的需求。其价格也是极为昂贵的，通常需要数万元以上。对多数本科生而言，此类笔记本的性能显然是溢出的。

按用途，我个人很不成熟地把计算机分为两类：

- **过渡机**：顾名思义，用来过渡的计算机，一般性能不算太强，主要用于满足具体确定将来方向之前的日常使用需求，以及在有多台生产力机之后作备用，或在这些机器之间进行一些轻量级的计算任务的过渡设备。
- **生产力**：生产力指的是性能较强或向某一方面特化的计算机，主要用于一些特定的任务（科研型计算、设计、高性能游戏等）。推荐生产力机买台式机，便于部件的替换升级和维护。

实际上大多数同学并不需要一台专门的生产力机，因为多数人并不需要进行上述没有很强的算力就干不了的特殊任务。

按CPU架构，计算机可以分为两类：amd64（大多数，通常也叫x86_64）和arm64（苹果）。由于苹果系列较为特殊，这里单独介绍，本章之后的攒机教程等都基于amd64架构的计算机进行讲解。

##### amd64

常规计算机大多搭载amd64架构的CPU，使得其兼容性极高，性能在各方面相对平衡，价格也相对低，适合各种使用场景。只要记得不要用轻薄本这类性能较低的机器来运行一些过重的计算任务（例如重型机器学习任务、图形设计、视频编辑、渲染等）就可以了。实际上我个人是比较建议购买此类计算机的：其性能和兼容性都比较好，价格也相对合理，适合大多数人的使用需求；如果机器系统希望升级也很容易，无需忍受arm64架构的计算机对其他系统（尤其是Linux）的支持不佳的问题。

##### arm64（Mac）

Mac自弃用Intel处理器后，转而使用自研的Apple Silicon系列芯片。该芯片使用arm64架构，单核性能极为优秀，可以与同代的amd64旗舰处理器媲美；多核持续性能略显不足，能效却极为优异。同时，因为CPU架构不同，所以很多软件需要对该架构进行适配才能在Mac上运行：虽然大部分常用软件都已经适配了Apple Silicon，但仍有一些软件（尤其是一些专业软件）可能存在兼容性问题，无法在Mac上运行或者性能较差。此外，作为类UNIX系统，macOS在Web开发、移动开发等较Windows更为简单，且macOS针对视频剪辑等功能有特殊的优化，因此表现出色；而在重型开发上（如dotnet开发、机器学习）则逊色于Linux和Windows。

Mac系列的另一个特点是其设计思路与生态系统。MacBook、iMac、Mac Mini等不同型号的Mac产品线覆盖了不同的使用场景和需求，用户可以根据自己的需求选择适合的型号。同时，Mac与其他苹果设备（如iPhone、iPad、Apple Watch等）之间如果能组成一个苹果生态，那么用户将会享受到极好的跨设备协同体验（例如在计算机甲上复制了一些内容，可以直接在计算机乙上粘贴；在iPhone上拍了一张照片，可以直接在MacBook上看到并使用），这也是Mac系列产品的一大卖点。较大的问题是，Mac的使用思路和Windows、Linux有显著的差异，如已经习惯了Windows或Linux的用户可能需要一定的适应期才能熟悉Mac的使用方式和操作习惯。

MacBook按上述的归类，应当归类到轻薄本中去：其续航长、性能较强，是出色的过渡设备。Mac Mini是一种极为轻小的台式机，性能很强，是作为个人服务器的不错选择。需要说明的是，这个服务器因为硬件上缺乏如RAID、ECC内存等的冗余设计，软件上macOS Server又已经被废弃了，且不能运行原生的Linux系统（arm64的Linux支持很差）或软件（例如Docker等），所以不适合传统生产力服务器，但用来娱乐、开发测试、挂载小型网页等绰绰有余。笔者的Mac Mini M4，能在同一台机器上同时$7\times 24$地长时间运行两个外挂了四百多个MOD的MineCraft服务器，性能完全可以支持，甚至还有余力。iMac则是一种一体机，性能更强。

Mac最大的缺陷是贵，以及维修和升级费用高昂。基础版的Mac Mini M4的价格大约是3700元，性价比极高，但加上一条内存以后，竟然涨到了5200元，突然就不那么"香"了；硬盘也是非常昂贵的。其次是生态封闭，用了一台Mac之后，硬件几乎只能被锁死在苹果生态内，例如极为昂贵的Apple键盘鼠标和显示器等，苹果对第三方设备的支持相对有限，笔者自己的高DPI显示器连接到Mac Mini M4虽然能亮，但不能像Linux或Windows一样对屏幕上的元素进行缩放，导致元素极小，最终被迫调低分辨率勉强使用。但Mac自己的高DPI显示器则可以随意缩放，至于价格，相对常规显示器而言，那真是天价了。软件生态同理，也相对封闭。建议在购买前先打听清楚：是否需要一些特定的专业软件来学习或工作？这些软件在Mac上是否有支持？性能如何？

如果无法确认这些问题的答案，建议还是购买一台常规的amd64架构的计算机来使用。

推荐有意向购买Mac的同学们前往[苹果官网](https://www.apple.com/cn/)查看最新的Mac系列产品信息，而且经过学生认证后，购买能有少量优惠（其实也就优惠大概200元），且售后有保障。
...
<span style="color: red; font-weight: bold;">如果你确实需要这本手册，那么我非常不建议你去线下任何门店购买任何计算机！</span>
...
SSD的速度和容量是关键。QLC便宜，但是掉速严重。系统盘建议TLC，仓库盘可以选择QLC。

HDD除非需要超大容量（4TB以上）或者仓库盘，否则不建议买。一定要买的话盯着CMR盘买，一般西数、希捷这两家就行。SMR叠瓦盘别碰。
...
|--------|--------|----------|----------|----------|
| 24英寸 | 1920×1080 | 92 | 60–70 cm | 办公、网课、MOBA |
| 27英寸 | 2560×1440 | 109 | 65–75 cm | 全能甜点 |
| 27英寸 | 3840×2160 | 163 | 55–65 cm | 代码、设计、4K 影音 |
| 32英寸 | 3840×2160 | 138 | 70–80 cm | 剪辑、影视后期 |
| 34英寸 | 3440×1440 | 110 | 65–75 cm | 带鱼屏游戏、股票 |

经验法则：1080p 别超过 24英寸，否则PPI太低，内容将非常模糊，对眼睛完全就是一种折磨。27英寸屏幕起步 2K；4K 最好 27–32英寸，否则缩放比例尴尬。带鱼屏（21:9）优先 3440×1440，2560×1080 纵向 PPI 太低。
...
然后开机。不要先进系统，一定要先进入 BIOS/UEFI 界面，检查 CPU、内存、硬盘等硬件信息是否正确，确认没有硬件故障；再检查风扇转速、温度等参数，确保散热系统工作正常。最后，检查硬盘通电次数和电池循环次数，一般小于10次可以接受，过高则可能是翻新机。如是二手交易则无需检查硬盘通电次数和电池循环次数。进入上述界面的方法因主板不同而有所区别，不过大多数主板都是 `F2` 或 `Delete` 键，具体参见主板或机器说明书，也可以留意开机时的提示信息。

#### 进系统

下一步，进入系统。在这个过程中务必不要连接网络，防止系统驱动更新、激活等操作影响退换货。Windows[^2]新机器默认情况下需要注册微软账户，不联网无法完成系统初始化。这时，可以在该初始化界面按下 `Shift+F10` 打开命令行，输入 `OOBE\BYPASSNRO`，然后重启电脑。这样就可以跳过联网注册微软账户的步骤。而在最新的Windows[^3]系统中，已经不支持上述本地账户登录手段了。对此，只能退而求其次，使用Live USB等工具临时验机。
...
- **刷新率、色域**：[UFO Test](https://www.testufo.com/) 网站跑 144/165/240 Hz，看是否掉帧；DisplayCAL 校色仪验证色域覆盖与 $\Delta E$。
...
!!! warning "警告"
    ...
    但是有一点是确定的：在创建账户的时候，用户名一定要使用英文和数字，不能使用中文，否则可能会导致例如GCC等软件无法正常安装和运行，这会在之后带来很多不必要的麻烦。

    这是因为，Windows系统采用的编码方案是UTF-16、而采取中文系统则非常可能沿用旧有的GBK编码方案；而GCC是从Linux移植过来的，只认UTF-8。这就导致了如果用户名中包含中文，那么在安装GCC等软件时，可能会出现路径问题，导致软件无法正常运行。但英文和数字不论是GBK、UTF-8还是UTF-16都是兼容ASCII的（即在这个区段，三种编码方案的编码结果是完全一样的），所以就不会出现上述问题了。具体好奇编码方案的同学可以把书往后翻。
...
[^1]: 个人使用寿命和HDD无明显差异，基本都能用到彻底换机。
[^2]: 主要是Win11。对Win10的支持于2025年10月14日终止。
[^3]: 指的是Windows 11 Beta Build 26120.6772 版和 Windows 11 Dev Build 26220.6772 版及其以后的版本。
