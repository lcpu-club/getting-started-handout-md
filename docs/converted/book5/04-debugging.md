...
有些时候部署的代码并不是直接从 Git 仓库中拉取的，而是经过打包、编译等步骤生成的二进制文件或者 Docker 镜像等。这时候，我们需要使用相应的工具来进行回滚操作。例如，如果我们使用 Docker 来部署服务，我们可以使用 `docker service update --image your_image:previous_version your_service` 命令来回滚到之前的镜像版本，也就是：
